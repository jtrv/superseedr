...
pub fn write_magnet(settings: &Settings, magnet_link: &str) -> io::Result<PathBuf> {
...
pub fn write_torrent_bytes(settings: &Settings, source_url: &str, bytes: &[u8]) -> io::Result<PathBuf> {
    let watch_dir = rss_watch_dir(settings)?;
