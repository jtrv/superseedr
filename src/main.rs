...
fn process_input(input_str: &str, watch_path: &Path) {
    if input_str.starts_with("magnet:") {
        ...
        tracing::info!("Attempting to write magnet link to temporary path: {:?}", temp_path);

        // 1. Write the content to the temporary file
        match fs::write(&temp_path, input_str.as_bytes()) {
            Ok(_) => {
                tracing::info!("Atomically renaming magnet file to final path: {:?}", final_path);
        ...
        match fs::canonicalize(&torrent_path) {
            Ok(absolute_path) => {
                ...
                tracing::info!("Attempting to write torrent path to temporary path: {:?}", temp_dest_path);
                
                // 1. Write the content to the temporary file
                match fs::write(&temp_dest_path, content) {
                    Ok(_) => {
                        tracing::info!("Atomically renaming path file to final path: {:?}", final_dest_path);
                        // 2. Atomically rename the temporary file to the final path
