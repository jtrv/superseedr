...
impl TokenBucket {
    pub fn new(capacity: f64, fill_rate: f64) -> Self {
        ...
        let (initial_tokens, initial_capacity) = if sane_fill_rate == 0.0 || !sane_fill_rate.is_finite() {
...
pub async fn consume_tokens(bucket_arc: &Arc<Mutex<TokenBucket>>, amount_tokens: f64) {
     if amount_tokens < 0.0 || !amount_tokens.is_finite() { return; }

     let (current_fill_rate, current_capacity) = {
         let bucket = bucket_arc.lock().await;
         if bucket.capacity.is_infinite() { return; }
     ...
     if current_fill_rate > 0.0 && current_fill_rate.is_finite() {
         if amount_tokens > current_capacity {
             ...
             if required_duration < Duration::from_secs(60 * 5) {
                 ...
                 eprintln!("Warning: Calculated sleep time for large request exceeds limit ({:?}).", required_duration);
             }
             return;
         }
...
mod tests {
    ...
    fn test_token_bucket_consume_success_direct() {
        ...
        assert!(bucket.tokens.abs() < TOLERANCE, "Expected tokens to be near zero, got {}", bucket.tokens);
    }
    ...
    fn test_token_bucket_consume_fail_direct() {
        ...
        assert!((bucket.tokens - 40.0).abs() < TOLERANCE, "Tokens should remain near 40.0, got {}", bucket.tokens);
    }
    ...
    async fn test_token_bucket_refill_direct() {
        ...
        bucket.refill();
        assert!((bucket.tokens - 20.0).abs() < TIMING_TOLERANCE, "Expected ~20.0 tokens, got {}", bucket.tokens);
        assert!(test_consume(&mut bucket, 15.0));
        assert!((bucket.tokens - 5.0).abs() < TIMING_TOLERANCE, "Expected ~5.0 tokens, got {}", bucket.tokens);
    }
    ...
    async fn test_token_bucket_refill_capacity_clamp_direct() {
        ...
        bucket.refill();
        assert!((bucket.tokens - 100.0).abs() < TIMING_TOLERANCE, "Expected ~100.0 tokens, got {}", bucket.tokens);
    }
    ...
    async fn test_consume_tokens_waits_for_refill_direct() {
        ...
        let target_wait = 0.5;
        assert!((elapsed.as_secs_f64() - target_wait).abs() < TIMING_TOLERANCE, "Expected ~{:.1}s sleep, got {:?}", target_wait, elapsed);
        ...
        assert!(bucket.lock().await.tokens.abs() < max_token_error, "Expected tokens near 0.0 (within {:.1}), got {}", max_token_error, bucket.lock().await.tokens);
    }
    ...
    async fn test_consume_tokens_large_request_direct() {
        ...
        assert!((bucket_locked.tokens - 100.0).abs() < TIMING_TOLERANCE, "Expected ~100.0 tokens after large request sleep, got {}", bucket_locked.tokens);
    }
    ...
    async fn test_consume_tokens_multiple_consumers_direct() {
        ...
        let task_1 = tokio::spawn(async move { consume_tokens(&bucket_1, 500.0).await; }); // Needs 0.5s
        let task_2 = tokio::spawn(async move { consume_tokens(&bucket_2, 1000.0).await; }); // Needs 1.0s
        ...
        assert!(elapsed.as_secs_f64() >= lower_bound && elapsed.as_secs_f64() < upper_bound, "Expected total time ~{:.1}-{:.1}s, got {:?}", lower_bound, upper_bound, elapsed);

        // Allow more tolerance for the final token count due to concurrency timing
        assert!(bucket.lock().await.tokens.abs() < TIMING_TOLERANCE * 10.0, "Expected tokens near 0.0 finally, got {}", bucket.lock().await.tokens);
    }
}
